...
select * from (select A.*, (select coalesce(sum(vote),0) from Comments as C where C.aid = A.aid) as votes from Assumptions as A where pid = 400004) as AV left join Objects using (oid) order by votes desc, AV.added;
